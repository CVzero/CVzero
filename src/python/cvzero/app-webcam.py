...
cv2.namedWindow('CVzero')

cv2.createTrackbar("colour_thresh", "CVzero",0,10,nothing)


while True:
    _, CVzero = cap.read()
    colour_thresh = cv2.getTrackbarPos("colour_thresh", "CVzero")
    # Add guide text to temp image
    out = CVzero.copy()
    cv2.putText(CVzero, "Press 'L' to enter Learn Mode ['q' to quit]", (10, 400), font, 0.75, (0, 100, 255), 1, cv2.LINE_AA)

    thresh = colour_thresh/100
